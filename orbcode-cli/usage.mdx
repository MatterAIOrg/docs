...
Files are loaded in this order (lowest precedence first): user memory (`~/.orbcode/AGENTS.md`), project memory (`AGENTS.md` and `.orbcode/AGENTS.md` in the cwd and parents), and local memory (`AGENTS.local.md`, private per-machine). Use `/init` to have OrbCode generate a starter `AGENTS.md` by analyzing the codebase. See the [Skills & memory page](/orbcode-cli/skills) for the discovery order and `@include` directives.
